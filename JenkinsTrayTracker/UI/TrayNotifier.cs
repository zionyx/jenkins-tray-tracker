    ...
    public partial class TrayNotifier : Component
    {
        ...
        private void notifyIcon_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            try
            {
                ...
                foreach (KeyValuePair<BuildStatusEnum, SortedSet<Project>> pair in projectsByStatus)
                {
                    ...
                    if (prefix != null)
                        text.Append(prefix);
                    string statusText = JenkinsTrayTrackerResources.ResourceManager
                ...
                if (string.IsNullOrEmpty(textToDisplay))
                    textToDisplay = JenkinsTrayTrackerResources.DisplayBuildStatus_NoProjects;
                notifyIcon.ShowBalloonTip(BALLOON_TOOLTIP_TIMEOUT, JenkinsTrayTrackerResources.DisplayBuildStatus_Caption,
        ...
        private void UpdateTrayTooltip(ICollection<Project> progressingAndErrorProjects)
        {
            ...
            if (progressingAndErrorProjects != null && progressingAndErrorProjects.Count > 0)
            {
                foreach (Project project in progressingAndErrorProjects)
                {
                    ...
                    {
                        tooltipText.Append(string.Format(JenkinsTrayTrackerResources.Tooltip_Failed_And_InProgress, project.Name));
                    }
                    else if (status.IsInProgress)
                    {
                        tooltipText.Append(string.Format(JenkinsTrayTrackerResources.Tooltip_InProgress, project.Name));
                    }
                    else
                    {
                        tooltipText.Append(string.Format(JenkinsTrayTrackerResources.Tooltip_BuildStatus, project.Name, status.Value.ToString()));
                    }
                    prefix = "\n";
            ...
            {
                tooltipText.Append(JenkinsTrayTrackerResources.Tooltip_AllGood);
            }
        ...
        private void UpdateBalloonTip(ICollection<Project> errorProjects, ICollection<Project> regressingProjects)
        {
                ...
                && errorProjects != null && errorProjects.Count > 0)
            {
                ...
                notifyIcon.ShowBalloonTip(BALLOON_TOOLTIP_TIMEOUT, JenkinsTrayTrackerResources.BuildFailed_Caption,
            ...
            else if (regressingProjects != null && regressingProjects.Count > 0)
            {
                ...
                notifyIcon.ShowBalloonTip(10000, JenkinsTrayTrackerResources.BuildRegressions_Caption,
        ...
        private void LoadIcon(BuildStatusEnum statusValue, bool isInProgress, bool isStuck)
        {
            ...
            try
            {
                string resourceName = string.Format("Jenkins.TrayTracker.Resources.TrayIcons.{0}.ico", status.Key);
